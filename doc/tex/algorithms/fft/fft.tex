...
\node [block, below of=input, node distance=3cm] (n) {n = Length of INPUT};
...
\node [block, below of=Radix-2, node distance=3cm] (output)  { OUTPUT $(Complex)$};
%% Draw edges
\path [line] (input) -- (n);
\path [line] (n) -- (decide);
...
\begin{figure}[ht]
	...
	\begin{center}
		...
		\node [block,below of=CT, node distance=2.5cm] (output) { OUTPUT $(Complex)$};
...
\begin{center}
	\begin{figure}[ht]
		...
		\begin{center}
			...
			\node [block,below of=postprocessing, node distance=2.5cm] (output) { OUTPUT $Complex$};
