...
In this section, we are going to detail an 1-out-2 OT Protocol's implementation using discrete variables (single photons) with polarization encoding.

The first step is to establish for both Alice and Bob the message length $s$. In this case, lets assume message's length $s=4$.
...
\end{tabular}
\end{table}
Alice and Bob also know the bit correspondence for each direction for each basis. For \textit{Rectilinear basis}, "$+$",
...
Only Alice knows information about messages $m_{0}$ and $m_{1}$.
In this case, lets assume the following two messages with size $s=4$, $m_{0} = \{0 0 1 1\}$ and $m_{1} = \{0 0 0 1\}$.

\begin{enumerate}
  \item Alice randomly generate a bit sequence with length $l=8$.
  ...
  \item Next, Alice sends to Bob throughout a quantum channel $l$ photons encoded using the basis defined in $S_{A1}$ and according to the bits defined in $S_{A2}$.
      ...
      $$S_{AB} = \{\uparrow, \nearrow, \searrow, \to, \to, \nearrow, \to, \searrow \} = \{90^{\circ}, 45^{\circ}, -45^{\circ}, 0^{\circ}, 0^{\circ}, 45^{\circ}, 0^{\circ}, -45^{\circ} \}.$$
  ...
  $$S_{B1} = \{ +,\times,+,\times,+,\times, \times, \times \}.$$
  Bob will get $s$ results:
  $$S_{B1'} = \{1,1,0,1,0,1,1,0 \}.$$

  \item Bob is going to send a hash value to Alice for each measurement that he performed. In this case, we are going to assume that the hash value is calculated using the \textit{SHA-256} algorithm \cite{Liu2009}. In detail, Bo has two sets $S_{B1}$ and $S_{B1'}$ and he is going to generate the set $S_{BH1}$ which have $l$ hash values calculated for each position of $S_{B1}$ with the correspondent position of $S_{B1'}$.


  \item Since Alice has received the confirmation of measurement from Bob,i.e after Alice has received $S_{BH1}$, she sends throughout a classical channel the basis which she has used to codify the photons, which in this case we assumed $S_{A1} = \{0,1,1,0,0,1,0,1\}$.
  ...
  \item In general, if we assume $l=n+\bar{n}$, Alice will ask Bob to open $\bar{n}-n+1$ pairs of the set that $\bar{n}$ corresponds. Next, Alice asks to open $n-1$ pairs of the set which $n$ corresponds.
       ...
       In this case, since Alice already knows how many basis Bob was correct and how many basis he was wrong about, she is going to ask Bob to open one of the wrong pairs and three of the right ones, i.e Bob sends to Alice the pairs that she wants and she will verify Bob's honesty with the hash function sent by him in step 4. At this time, Alice deleted from Bob sets one wrong bit and three right bits thereby being the same number of right positions and wrong positions, i.e Bob has two position bits right and two positions wrong. At this time, we are at $t=4$ in figure \ref{alicebobkeys}.

      In detail, Bob and Alice are being continuously developing a set of keys in which they always have the same number of wrong and right position bits. In order to better understand this, lets assume this specific example by analysing figure \ref{alicebobkeys}. In this figure there are two sets which are being built continuously, i.e as Alice sends photons to Bob throughout a quantum channel, he measured them with the random basis that he chose, she sends to him the basis she used to encode the photons and at this time he knows in which measurements he was correct. In this case, he sends to Alice the number 3 of wrong bits and she ask to open one pair of wrong values and three pairs of right values according with the commitment done by Bob in the previous step 4. In this way, Bob and Alice add to them sets the positions that she not ask to destroy in order to have a set with the same number of right and wrong values. As we can see in figure \ref{alicebobkeys} they know that at $t=4$ the number of right pairs and wrong pairs are the same and this condition is verified. After that, the previous procedure is being performed again until $t=10$ when they know the same again.

      They continuously do this procedures during all protocol, i.e them sets are built simultaneously with the other message's change throughout the classical channels.
