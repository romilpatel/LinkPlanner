...
\subsubsection{Requirements for the KK receiver scheme}

The Kramers-Kronig receiver based communication scheme relies on identifying a condition which ensures that \textbf{the received signal is minimum phase}, in such a case its phase can be uniquely extracted from its intensity.\\ In order to facilitate the extraction of phase information from the intesity of the received minimum phase signal, it is prerequisite that the transmitted signal should be \textbf{analytical signal} in which real and imaginary parts are related to each other by means of Hilbert transform of each other.   

\subsubsection{What is minimum phase signal?}
...
\subsubsection{what is Analytical Signal?}
An analytic signal is a complex-valued function that has no negative frequency components and its real and imaginary parts are related to each other by the Hilbert transform.
\begin{equation}
s_a(t)=s(t)+j\hat{s}(t)
