...
\label{5.15}
\end{equation}
the real part $E_{s,r}(t)$  and the imaginary parts $E_{s,i}(t)$ are Hilbert transform of each other (See Appendix ).
\begin{equation}
\begin{split}
...
\label{Eq 5.40}
\end{equation} 
then $\tilde{G}(\omega)$, the spectrum of $G(t)$, is such that $G(\omega)=0$ for $\omega<0$.
Under the hypothesis stated by Equation \ref{Eq:5.17},we can write $G(t)$ as,
\begin{equation}
\begin{split}
...
\label{Eq 5.41}
\end{equation}
From the equations \ref{Eq 5.40} and \ref{Eq 5.41},
\begin{equation}
\begin{split}
...
\label{Eq 5.42}
\end{equation}
Comparing Imaginary part of Equation \ref{Eq 5.42},
\begin{equation}
\begin{split}
...
\label{Eq 5.43}
\end{equation}
In equation \ref{Eq 5.43}, $\frac{1}{\pi} p.v. \int_{-\infty}^{\infty} \frac{log|\bar{E}|}{t-t'} dt'=0$ which leads to,
\begin{equation}.
\begin{split}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Appendix E}\\
If we consider a filter $H(\omega)$, described in Figure, that has a unity magnitude response for all frequencies. Also, the phase response is $-\pi/2$ for all positive frequencies and $\pi/2$ for negative frequencies. The transfer function of this filter is,
\begin{equation}
\begin{split}
