...
bool Photodiode::runBlock(void){
  ...
  /*normal_distribution<double> distribution(0, 1);
	...
	for (int i = 0; i < process; i++) {
		...
		//t_real noise1 = distribution(generator1);
		//t_real noise2 = distribution(generator2);
		...
		//t_real power1withnoise = power1 + noise1*sqrt((power1*h*SPEED_OF_LIGHT) / (dt*wvlgth)) + noise1* noise1 * h*SPEED_OF_LIGHT / (4 * dt*wvlgth);
		...
		//t_real power2withnoise = power2 + noise2*sqrt((power1*h*SPEED_OF_LIGHT) / (dt*wvlgth)) + noise2* noise2 * h*SPEED_OF_LIGHT / (4 * dt*wvlgth);
